...
module.exports = {
  ...
  async lista (req, res) {
    try {
      ...
      const conversor = new ConversorPost('json', req.acesso ? (req.acesso.todos.permitido ? req.acesso.todos.atributos : req.acesso.apenasSeu.atributos) : [])
      if(!req.estaAutenticado){
        posts = posts.map(post => {
          post.conteudo = post.conteudo.substring(0, 10) + '... Você precisa assinar o blog para ler o restante do post'
          return post
        })
      }
